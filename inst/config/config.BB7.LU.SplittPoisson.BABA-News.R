...
load(file.path(R_CPL_LIB_ROOT_DIR, "data/BABA-News.Rdata"))

## No. of Total Observations
nObsRaw <- length(Y[[1]])

## Data subset used
nObsIdx <- (1 + 200-nObsRaw):nObsRaw
...
Mdl.parLink[[2]][["mu"]] <- list(type = "log", nPar = 1, nPar = 1)
...
varSelArgs[[3]][["lambdaL"]] <- list(cand = 2:19,
                                     init = "all-in")
varSelArgs[[3]][["lambdaU"]] <- list(cand = 2:19,
                                     init = "all-in")
