...
#load(file.path("fli", "data/stock.Rdata"))
#source(file.path("fli", "inst/scripts/Get-RISK-CONTAGION.R"))
...
myfun = function(x, idx)x[idx, ,drop = FALSE]
Mdl.Y <- lapply(Y[Mdl.MargisNM[-length(Mdl.MargisNM)]], myfun, Mdl.dataUsedIdx)
