...
Mdl.X <- MCMCUpdate
Mdl.X[[1]][["mu"]] <- cbind(1, X[[MargisNM[1]]][, 1:9])[nObsIdx, 1:3, drop = FALSE]
Mdl.X[[1]][["phi"]] <- cbind(1, X[[MargisNM[1]]][, 1:9])[nObsIdx, 1:1, drop = FALSE]
Mdl.X[[1]][["df"]] <- cbind(1, X[[MargisNM[1]]][, 1:9])[nObsIdx, 1:1, drop = FALSE]
Mdl.X[[1]][["lmd"]] <- cbind(1, X[[MargisNM[1]]][, 1:9])[nObsIdx, 1:1, drop = FALSE]

Mdl.X[[2]][["mu"]] <- cbind(1, X[[MargisNM[2]]][, 1:9])[nObsIdx, 1:1, drop = FALSE]
Mdl.X[[2]][["phi"]] <- cbind(1, X[[MargisNM[2]]][, 1:9])[nObsIdx, 1:1, drop = FALSE]
Mdl.X[[2]][["df"]] <- cbind(1, X[[MargisNM[2]]][, 1:9])[nObsIdx, 1:1, drop = FALSE]
Mdl.X[[2]][["lmd"]] <- cbind(1, X[[MargisNM[2]]][, 1:9])[nObsIdx, 1:1, drop = FALSE]

Mdl.X[[3]][["tau"]] <- cbind(1, X[[MargisNM[1]]][, 1:9], X[[MargisNM[2]]][, 1:9])[nObsIdx, 1:3, drop = FALSE]
