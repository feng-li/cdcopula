...
target1="^GOLD"
target2="^CORN"
load(file.path(CDCOPULA_LIB_ROOT_DIR, sprintf("data/gold1/%s-%s-20181103.Rdata",substr(target1,2,nchar(target1)),substr(target2,2,nchar(target2)))))
Mdl.MargisType <- c("TEIGEN", "TEIGEN", "BB7")
Mdl.MargisNM <- c(target1, target2, "BB7")
...
Mdl.X[[3]][["lambdaL"]] <- cbind(1, X[[1]][Mdl.dataUsedIdx, 1:n_col], X[[2]][Mdl.dataUsedIdx, 1:n_col])#[, 1, drop = FALSE]
## Mdl.X[[3]][["lambdaU"]] <- cbind(1, X[[1]][Mdl.dataUsedIdx, 1:9], X[[2]][Mdl.dataUsedIdx, 1:9])#[, 1, drop = FALSE]
Mdl.X[[3]][["lambdaU"]] <- cbind(1, X[[1]][Mdl.dataUsedIdx, 1:n_col], X[[2]][Mdl.dataUsedIdx, 1:n_col])#[, 1, drop = FALSE]
