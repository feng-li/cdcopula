...
kendalltau <- function(CplNM, parCpl)
{
    ...
    else if (tolower(CplNM0) %in% c("gaussian", "mvt"))
    {
        rho <- parCpl[["rho"]]
        out <- 2/pi*asin(rho)
    }
...
## kendalltau0 <- function(CplNM0, parCpl)
##   {
##     if(tolower(CplNM0) == "bb7")
##       {
##         theta <- parCpl[["theta"]]
...
## tauTol <- 0.01
## tauTabular <- kendalltauTabular(CplNM0, tol = tauTol)

## parCpl <- list(delta = delta, theta = NA)



## parCplOut <- kendalltauInv(CplNM0, parCpl, tau, tauTabular, parCaller = "theta")
