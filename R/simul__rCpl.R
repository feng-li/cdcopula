...
rCpl <- function(n, parCpl, CplNM, exArgs = NA)
{
    ...
    if(tolower(CplNM0) == "bb7") # Joe 1997
    {
        ## Subtract the parameters list.
        delta <- matrix(parCpl[["delta"]], n, 1)
        theta <- matrix(parCpl[["theta"]], n, 1)
        ...
        v <- matrix(runif(n*p, 0+tol, 1-tol), n, p)
        u <- matrix(NA, n, p)
        u[, 1] <- v[, 1]
    ...
    else if(tolower(CplNM0) == "mvt") ## the multivariate t Demarta Mcneil (2005)
    {
        ...
        x <- matrix(rmvt(n = n*p, df = df, sigma = corr, type = "shifted",
                         method = "chol"), n, p)
    ...
    else if(tolower(CplNM0) == "clayton")
    {
        ...
        applyfun <- function(i, par)
        {
            rCopula(n = 1, copula = archmCopula(family = "clayton", dim = 2, param = par[i]))
        }

        u = t(sapply(1:n, applyfun, par = delta))
        out <- list(u = u)
    }
    else
    {
        stop("Not implemented for given copula name.")
    }
    return(out)
}
