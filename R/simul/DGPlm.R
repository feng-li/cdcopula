...
DGPlm <- function(Y, beta, Xlim, intercept = TRUE)
{
    ...
    ## browser()
    ## Variational Y with constant X is possible when intercept in.
    if(betaIdxLen.NonZero == 0 & intercept & var(Y)> .Machine$double.eps)
    {
        warning("Variational response variable with constant covariates are generated!")
    }
    ...
    {
        ## Do nothing, X (except the intercept) is filled with fake covariates
    }
    ## print(X[1, ])
    return(X)
}
