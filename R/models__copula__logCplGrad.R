...
logCplGrad <- function(CplNM, u, parCpl, parCaller)
{
    ...
    if(tolower(CplNM0) == "bb7")
    {
        ...
        {
            gradFun4delta <- function(u.r, delta, theta)
            {
                T1 <- 1-(1-u.r)^theta
                Tu1 <- T1[, 1]
                Tv1 <- T1[, 2]
            ...
            gradout <- gradFun4delta(u.r = u.r, theta = theta, delta = delta)
            ...
            if(any(redo.idx))
            {
                ...
                gradout.redoMPFR <- gradFun4delta(u.r = mpfr(u.r[redo.idx, , drop = FALSE], precBits = precBits),
...
################################################################################
            ...
            gradFun4theta <- function(u.r, theta, delta)
            {
                ...
                SD12 <- rowSums((T1[, 2:1])^(1+delta)*(1-u.r)^theta*log(1-u.r))
                SD34 <- rowSums(T1^(-1-delta)*(1-u.r)^theta*delta*log(1-u.r))
                ...
                out <- (1/(L1^3*(-1-delta*theta+L1^(1/delta)*(1+delta)*theta))*
                        PT1^(-1-2*delta)*(rowSums(T1^delta)-PT1^delta)^2*(
                            1/theta*PT1^(-delta)*(-SD12*(1+delta)*theta*(
                                ...
                                L1*(-1-delta*theta+
                                    L1^(1/delta)*(1+delta)*theta)*
                                (rowSums(T1[, 2:1]*(T1+(1-u.r)^theta*(1+delta))*log(1-u.r)))))
                return(out)
            }

            ## The chain gradient
            gradout <- gradFun4theta(u.r = u.r, theta = theta, delta = delta)
            redo.idx <- (!is.finite(gradout))
            if(any(redo.idx))
            {
                require("Rmpfr", quietly = TRUE)
                precBits <- 1024
                ## MPFR class used for u, theta,  delta
                gradout.redoMPFR <- gradFun4theta(u.r = mpfr(u.r[redo.idx, , drop = FALSE], precBits = precBits),
                                                  theta = mpfr(theta[redo.idx], precBits = precBits),
                                                  delta = mpfr(delta[redo.idx], precBits = precBits))
                gradout.redo <- as.numeric(gradout.redoMPFR)
                gradout[redo.idx] <- gradout.redo
...
################################################################################
            ...
            out[["u"]] <- gradCpl.u*logCplGrad4u2rotate(imar = imar, rotation = rotation)
        }
    ...
    else if((tolower(CplNM0) == "sjc"))
    {
        ...
        if("delta" %in% tolower(parCaller))
        {
            d1 <- dCpl(CplNM = "BB7", u.r, parCpl = parCpl, log = FALSE)
            d2 <- dCpl(CplNM = "BB7", 1-u.r, parCpl = parCpl, log = FALSE)

            chain1 = 1/(d1+d2)

            chain2 <- (d1*logCplGrad(CplNM = "BB7", u = u.r, parCpl = parCpl,
                                     parCaller = parCaller)[["delta"]] +
                       d2*logCplGrad(CplNM = "BB7", u = 1- u.r, parCpl = parCpl,
                                     parCaller = parCaller)[["delta"]]) # n-by-1

            out[["delta"]] <- chain1*chain2
        ...
        if( "theta" %in% tolower(parCaller))
        {
            ...
            chain2 <- (d1*logCplGrad(CplNM = "BB7", u = u.r, parCpl = parCpl,
                                     parCaller = parCaller)[["theta"]] +
                       d2*logCplGrad(CplNM = "BB7", u = 1- u.r, parCpl = parCpl,
                                     parCaller = parCaller)[["theta"]]) # n-by-1

            out[["theta"]] <- chain1*chain2
        ...
        if(any(paste("u", 1:q, sep = "") %in% tolower(parCaller)))
        {
            ...
            chain2 <- (d1*logCplGrad(CplNM = "BB7", u = u.r, parCpl = parCpl,
                                     parCaller = parCaller)[["u"]] -
                       d2*logCplGrad(CplNM = "BB7", u = 1-u.r, parCpl = parCpl,
                                     parCaller = parCaller)[["u"]]) # n-by-1
    ...
    else if(tolower(CplNM0) == "mvt")
    {
        ...
        u.quantile <- qt(u.r, df) # the x in t(x, df)
        ...
        if(any(paste("u", 1:q, sep = "") %in% tolower(parCaller)))
        {
            ...
            if(any(I))
            {
                ...
                F1x1.I <- (-(2*(x1.1-mu)^3/((x1.1-mu)^2+df.1)^2-2*(x1.1-mu)/((x1.1-mu)^2+df.1))*
            ...
            FUN <- function(i, x, mu, df, rho, uIdx)
            {
                ...
                out <- (-(v+p)/2*C2^(-1)*1/v*(2*solve(Sigma)%*%(x_i-mu)))
                return(out)
            }
            ...
            logCplGrad.u1 <- gradLogCpl.x1*(1/F1x1)- 1/fx1*f1x1/F1x1

            out[["u"]] <-  logCplGrad.u1*logCplGrad4u2rotate(imar = imar, rotation = rotation)
        }
    }
    else if(tolower(CplNM0) == "gumbel")
    {
        ...
        if(any(paste("u", 1:q, sep = "") %in% tolower(parCaller)))
        {
            ...
            u1 <- u.r[, 1]
            logCplGrad.u1 <- (-1/u1 + ((-1+delta - (1+uDelta^(2/delta)
    ...
    else if(tolower(CplNM0) == "clayton")
    {
        ...
        if(any(paste("u", 1:q, sep = "") %in% tolower(parCaller)))
        {
            ...
            logCplGrad.u1 <- (-(u2^delta*delta + A*(1+delta))/(u1*(-u2^delta+ A)))
...
#' @export
logCplGrad4u2rotate <- function(imar, rotation)
{
    if(imar == 1 & rotation == 90)
    {
        out <- -1
    }
    else if(imar == 2 & rotation == 270)
    {
        out <- -1
    }
    else
