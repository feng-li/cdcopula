...
PropGNewtonMove <- function(MCMC.propArgs, Mdl.varSelArgs, Mdl.priArgs, betaIdxProp, parUpdate,
                            Mdl.Y, Mdl.X, Mdl.beta, Mdl.betaIdx, Mdl.parLink, Mdl.algorithm,
                            Mdl.MargisType, staticCache, MCMC.UpdateStrategy)
{
    ...
    if(any(is.nan(betaCurr) || any(is.infinite(betaCurr))))
    {
        stop("NA/NaN/Inf are not allowed for `beta`.")
    }
...
###----------------------------------------------------------------------------
        ...
        logDensGradHess.prop <- logDensGradHess(
            ...
            Mdl.Y = Mdl.Y, Mdl.parLink = Mdl.parLink,
            parUpdate = parUpdate,
        ...
        ## The general Newton's update is in progress
        if((iStep <= kSteps))
        {
            ## update the proposed parameters via the general Newton formula. This is a
            ## robust solution when `HessObs.pp.Inv` is inverted via the generalized
            ## inverse function ginv()
        ...
        {
            ## (k+1):th step (Update is done).  Make a output
    ...
    ## if(any(is.nan(param)) || any(is.infinite(param))) errorFlag  =  TRUE
