...
kendalltauInv <- function(CplNM, parCplRep, method = c("tabular", "iterative")[1])
{
  if(tolower(method) == "tabular")
  {
    ...
    {
      assign("tauTabular",
             kendalltauTabular(CplNM = CplNM, tol = 1e-2),
             envir = .GlobalEnv)
    }
...
{
  ## TODO: The max interval could not handle Inf in the uniroot function.  TODO: Consider
  ## the error handle. i.e., In theory (see the Appendix in the paper) you can't have
  ## small tau with big delta (lower tail dependent) which yields non accurate root.
  ...
  if(tolower(CplNM0) == "bb7")
  {
    ...
    else if(tolower(parCaller) == "delta")
    {
      ...
      for(i in 1:parLen)
      {
        ...
        thetaCurr <- theta[i]
        outRootCurr <-
        try(uniroot(function(x, thetaCurr, deltaCurr, tauCurr)
        {
          kendalltau(CplNM = CplNM,
