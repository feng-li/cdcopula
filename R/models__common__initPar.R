...
initPar <- function(Mdl.varSelArgs, Mdl.priArgs, Mdl.betaInit, Mdl.X, Mdl.Y, Mdl.MargisType,
                    Mdl.parLink, Mdl.algorithm, MCMC.Update, MCMC.optimInit)
{
...
###----------------------------------------------------------------------------
       ...
       any(tolower(unlist(Mdl.betaInit)) == "random"))
    {
        ...
        else
        {
            ...
            nOptim.Sample <- min(round(nObs*MCMC.optimInit), nObs)
        ...
        if(exists("Mdl.Y", inherits = FALSE))
        {
...
###----------------------------------------------------------------------------
            ...
            if(class(varSelCandConfigCurr) == "character")
            {
                ...
                if(any(is.na(c(cand1, cand2))))
                {
                    stop("Variable selection candidates are not specified correctly.")
                }
                else
                {
                    varSelCandCurr <- cand1:cand2
                }
            }
            else
            {
